Given("that my resume with software development experience has been parsed and is ready to be matched") do
...
When("the matching algorithm is complete") do
  @user.resumes.first.get_job_matches_spacey
end

Then("the top job match will be for a software development job") do
  ...
  expect(match_engineering_job.score).to be > match_reporting_job.score
end
